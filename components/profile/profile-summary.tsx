...
export function ProfileSummary({
  ...
  return (
    <div className="relative border-b border-[#eff3f4] px-4 pb-4">
      <div className="absolute -top-[68px] left-4 overflow-hidden rounded-full border-4 border-white bg-neutral-200 shadow-sm">
        <UserAvatar
          src={avatarUrl}
          name={displayName}
      ...
      <div className="flex justify-end pt-3">
        {editProfileHref ? (
          ...
          <ProfileFollowButton
            key={`${followTargetUsername}-${followedByMe}`}
            targetUsername={followTargetUsername}
            initialFollowed={Boolean(followedByMe)}
          />
        ) : null}
      </div>
